...
<nav class="navbar navbar-expand-lg bg-body-tertiary mb-3">
	...
	<h3>Welcome to your profile, <c:out value="${user.username}"/></h3>
	...
	<table class="table table-striped table-hover border table-bordered">
			...
			<tbody>
				<c:forEach var="oneChar" items="${user.characters}">
				<tr>
					<td> <c:out value="${oneChar.name}"/></td>
					<td> <c:out value="${oneChar.charaClass}"/> </td>
					<td> <c:out value="${oneChar.level}"/></td>
					<td> <a href="/characters/${oneChar.id}"> View</a> |
						 <a href="/characters/${oneChar.id}/edit"> Edit</a> <br />
						 <form action="/characters/${oneChar.id}" method="post">
